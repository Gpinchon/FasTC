...
public:
    ...
    void Join()
    {
        ...
        if (result == WAIT_FAILED) {
            ErrorHandler(LPTSTR("WaitForSingleObject"));
        }
    }
};
...
public:
    TCThreadImplFactory(TCCallable& callable)
        : m_Callable(callable)
    {
    }
...
{
    if (!SwitchToThread()) {
        ErrorHandler(LPTSTR("SwitchToThread"));
    }
}
...
TCMutex::TCMutex()
    : TCThreadBase(TCMutexImplFactory())
{
}
...
public:
    TCLockImplFactory(TCMutex& mutex)
        : m_Mutex(mutex)
    {
    }
...
TCLock::TCLock(TCMutex& mutex)
    : TCThreadBase(TCLockImplFactory(mutex))
{
}
...
public:
    ...
    void Wait(TCLock& lock)
    {
        ...
        if (!result) {
            ErrorHandler(LPTSTR("SleepConditionVariableCS"));
        }
    }
