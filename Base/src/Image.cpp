...
template <typename T>
inline T PixelAbs(const T& a);
template <>
inline FasTC::IPixel PixelAbs<FasTC::IPixel>(const FasTC::IPixel& p)
{
    return FasTC::IPixel(fabs(p[0]));
}

template <>
inline FasTC::Pixel PixelAbs<FasTC::Pixel>(const FasTC::Pixel& p)
{
...
template <>
inline FasTC::Color PixelAbs<FasTC::Color>(const FasTC::Color& p)
{
...
#ifdef _MSC_VER
template <typename T>
T log2(T x) { return static_cast<T>(log((long double)x) / log(2.0)); }
#endif
...
{
    if (pixels) {
        m_Pixels = { pixels, pixels + GetNumPixels() };
    }
}
...
double Image<PixelType>::ComputeSSIM(Image<PixelType>* other)
{
    ...
    */
    ...
    if (img1.GetWidth() < filterSz || img1.GetHeight() < filterSz || img2.GetWidth() < filterSz || img2.GetHeight() < filterSz) {
        return -1.0;
    }
    ...
    for (uint32 j = 0; j < h; j++) {
        for (uint32 i = 0; i < w; i++) {
            ...
            double ssim = ((2.0 * m1m2 + C1) * (2.0 * s1s2 + C2)) / ((m1sq + m2sq + C1) * (s1sq + s2sq + C2));
...
double Image<PixelType>::ComputeMeanLocalEntropy()
{
    ...
    for (uint32 j = kHalfKernelSz; j < GetHeight() - kHalfKernelSz; j++) {
        for (uint32 i = kHalfKernelSz; i < GetWidth() - kHalfKernelSz; i++) {
            ...
            entropyIdx(i - kHalfKernelSz, j - kHalfKernelSz) = static_cast<float>(subImg.ComputeEntropy());
        }
    }
...
void Image<PixelType>::SetImageData(uint32 width, uint32 height, PixelType* data)
{
    if (!data) {
        ...
        m_Width = width;
        m_Height = height;
        m_Pixels = { data, data + GetNumPixels() };
    }
}
