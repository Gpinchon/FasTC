...
bool ImageLoaderPNG::ReadData()
{
    ...
    if (1 != png_get_IHDR(png_ptr, info_ptr, (png_uint_32*)(&m_Width), (png_uint_32*)(&m_Height), &bitDepth, &colorType, NULL, NULL, NULL)) {
    ...
    case PNG_COLOR_TYPE_PALETTE: {
        ...
        m_RedData.resize(numPixels);
        m_GreenChannelPrecision = bitDepth;
        m_GreenData.resize(numPixels);
        m_BlueChannelPrecision = bitDepth;
        m_BlueData.resize(numPixels);

        png_colorp palette;
        int nPaletteEntries;
        png_uint_32 ret = png_get_PLTE(png_ptr, info_ptr, &palette, &nPaletteEntries);
        if (ret != PNG_INFO_PLTE) {
            std::fill(m_BlueData.begin(), m_BlueData.end(), 0);
            std::fill(m_RedData.begin(), m_RedData.end(), 0);
            std::fill(m_GreenData.begin(), m_GreenData.end(), 0);
