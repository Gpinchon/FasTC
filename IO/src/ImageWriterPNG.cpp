...
public:
        ...
        png_size_t byteCountToWrite)
    {
        ...
        while (writer.m_StreamPosition + byteCountToWrite > writer.m_RawFileData.size()) {
            writer.m_RawFileData.resize(writer.m_RawFileData.size() << 1);
        }
...
bool ImageWriterPNG::WriteImage()
{
    ...
    m_RawFileData.resize(m_StreamPosition);
    return true;
}
