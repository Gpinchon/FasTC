...
#define LOAD(x)                      \
    ...
    if (m_Processor) {
        ...
        while (rdr.GetData() < imgData) {
            ...
            std::vector<uint8> keyValueData(keyAndValueByteSize);
            ...
            // Read the bytes...
            std::copy(rdr.GetData(), rdr.GetData() + keyAndValueByteSize, keyValueData.begin());
            auto key = keyValueData.begin();
            auto value = key;
            while (*value != '\0') {
                value++;
            }
            ...
            m_Processor((const char*)&*key, (const char*)&*value);
