...
Image::Image(uint32 width, uint32 height)
    ...
    , m_FractionalPixels(width * height)
{
    assert(width > 0);
    assert(height > 0);
}
...
Image::Image(const Image& other)
    ...
    , m_FractionalPixels(other.m_FractionalPixels)
{
}
    ...
    EWrapMode wrapMode)
{
    ...
    std::vector<FasTC::Pixel> upscaledPixels(newWidth * newHeight);

    m_FractionalPixels.resize(newWidth * newHeight);
    ...
    SetImageData(newWidth, newHeight, upscaledPixels.data());
}
...
void Image::AverageDownscale(uint32 xtimes, uint32 ytimes)
{
    ...
    std::vector<Pixel> downscaledPixels(newWidth * newHeight);
    ...
    SetImageData(newWidth, newHeight, downscaledPixels.data());
}
    ...
    EWrapMode wrapMode, bool bOffsetNewPixels)
{
    ...
    std::vector<FasTC::Pixel> downscaledPixels(newWidth * newHeight);
    ...
    // Allocate memory
    std::vector<float> imgData(19 * w * h);
    float* I = imgData.data();
    float* Ix[5] = {
        &imgData.at(w * h),
        &imgData.at(2 * w * h),
        &imgData.at(3 * w * h),
...
#define CPNT(dx) Pixel::ConvertChannelToFloat(static_cast<uint8>(GetPixels()[dx].Component(c)), bitDepth[c])
                ...
                Ixy[c][idx] = (CPNT(upidx) - CPNT(xphidx) - CPNT(yphidx) + 2.0f * CPNT(idx) - CPNT(xmhidx) - CPNT(ymhidx) + CPNT(downidx)) / 2.0f;
#undef CPNT
            }
        }
    }
    ...
    for (uint32 j = 0; j < newHeight; j++) {
        for (uint32 i = 0; i < newWidth; i++) {
            ...
            for (uint32 c = 0; c < 4; c++) {
                ...
                if (fabs(denom) > 1e-6) {
                    It -= (Ixsq * Ixx[c][idx] + 2 * Ix[4][idx] * Iy[idx] * Ixy[c][idx] + Iysq * Iyy[c][idx]) / denom;
                }
...
{
    std::vector<uint32> outPixels(GetWidth() * GetHeight());
    const uint8 fullDepth[4] = { 8, 8, 8, 8 };
    for (uint32 j = 0; j < GetHeight(); j++) {
        for (uint32 i = 0; i < GetWidth(); i++) {
            FasTC::Pixel p = (*this)(i, j);
    ...
    FasTC::Image<> img(GetWidth(), GetHeight(), outPixels.data());
